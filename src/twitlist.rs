...
use std::iter::{FromIterator, FusedIterator, Extend};
use std::marker::PhantomData;
use std::mem;
use std::ops::{BoxPlace, InPlace, Place, Placer};
...
impl<T> LinkedList<T> {
    ...
    pub fn remove(&mut self, n: usize) -> Option<T> {
        if self.len <= n { None }
            ...
            unsafe {
                let ref mut node = self.head.unwrap();
                for i in 0..n-1 {
                ...
                Some(Node::into_element(box node))
            }
        }
    }
